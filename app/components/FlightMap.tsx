...
export default function FlightMap({ initialFlights }: Props) {
  ...
  useEffect(() => {
    ...
    for (const [, { airport, flights: grounded }] of groundedGroups) {
      ...
      // Hover — simple preview tooltip
      marker.bindTooltip(`
        <div style="font-family:sans-serif;font-size:12px;line-height:1.6">
          <strong>${airport.iata} · ${airport.city}</strong><br/>
          <span style="color:#8b949e">${grounded.length} aircraft on ground</span><br/>
          <span style="color:#5bbd8a;font-size:11px">Click to view all</span>
        </div>
      `, { direction: 'top', offset: [0, -16] })

      // Click — open side panel
      marker.on('click', () => {
        setSelectedAirport({ airport, flights: grounded })
      })
