// app/components/AirlineIcon.tsx
'use client'
...
export default function AirlineIcon({ callsign, size = 56 }: Props) {
  const airline = getAirlineInfo(callsign)
  const color = airline?.color ?? '#b8b8b8'

  return (
    <div
      className="rounded-md flex items-center justify-center relative flex-shrink-0 overflow-hidden"
    ...
    >
      ...
      <div
        className="rounded-md items-center justify-center text-white p-2"
        style={{
          width: size * 0.8,
          height: size * 0.8,
          background: color,
